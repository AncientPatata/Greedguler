...
if __name__ == "__main__":
    with open("schedule.json") as file_handle:
        ...
        for (index, machine_jobs) in enumerate(schedule):
            ...
            job_list += [{"job_index": job["job_index"],"start_time": datetime.fromtimestamp(job["start_time"]), "end_time": datetime.fromtimestamp(job["end_time"]), "machine": machine} for job in machine_jobs] #[dict(job, **{"machine": machine}) for job in machine_jobs[:10] ]
